...
//for ($i = 1; $i < 0x100; $i++) {
//	print "mknote ".fmthexnum($i).": ".mknote($i)."\r\n";
//}
//die;
	...
	if ($cmd >= 0xE0) {
		...
		switch ($cmdid) {
			...
			case 0x07:
				...
				$ptrLow = getdb($i++);
				$ptrHigh = getdb($i);
				out("snd_loop \${$timerId}, {$timerVal}, \${$ptrHigh}{$ptrLow} ; L1F{$ptrHigh}{$ptrLow}"); 
				break;
			case 0x08:
				$x = hexdec(getdb($i));
				$time = ($x >> 4) & 0b111;
				$inc = ($x >> 3) & 0b1;
				$num = $x & 0b111;
				out("sweep $time, $inc, $num");
				break;
			case 0x09:
				$val = getdb($i);
				out("panning ".generate_const_label($val, $nr51));
				break;
			case 0x0C:
				$ptrLow = getdb($i++);
				$ptrHigh = getdb($i);
				out("snd_call \${$ptrHigh}{$ptrLow} ; L1F{$ptrHigh}{$ptrLow}"); 
				break;
			case 0x0D:
				out("snd_ret");
				$incPtr = false;				
			...
			case 0x0E:
				...
				switch ($chnum) {
					...
					case 4:
						...
						break;
					default:
						out("sndnrx1 \$".getdb($i)." ; THIS SHOUDN'T BE OUTPUT {$chnum}");
				}
			...
			case 0x1A:
				...
				break;		
			case 0x1C:
				$note = mknote(hexdec(getdb($i++)));
				$len = getdb($i);
				out("pitch_slide {$note}, \$".fmthexnum($len));
				break;				
		}
...
function out($txt) {
	//print $txt;
	global $h, $unused_marker, $label;
	fwrite($h, "$label\t".$txt.$unused_marker."\r\n"); 
}
function outraw($txt) {
	global $h;
	fwrite($h, $txt); 
